...
<# 
    ...
    Vous devez télécharger le fichier CTRQ-100K_CTRQ-100K_COVER_SO_TEL.zip vous-même sur la Geoboutique du Québec et le copier dans le répertoire sources (créez le répertoire s'il n'existe pas).
#>
...
if ($télécharger)
{
    ...
    if ($obtenirTelephone){
        ...
        Invoke-WebRequest -Uri $UriTelephoneurg -Outfile "$($QOSMTelechargements)\telephone_urg.zip"
    }
    if ($obtenirVilles){
        "...Villes"
    ...
    if ($obtenirTerresAutochtones){
        "...Territoires autochtones"
        Invoke-WebRequest -Uri $UriTerresAutochtones -Outfile "$($QOSMTelechargements)\terres_autochtones.zip"
    }
